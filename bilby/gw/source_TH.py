...
def TH_phase_corr(f1, mass_1, mass_2, a_1, a_2):
	mass_1_1, mass_1_2 = mass_1*time_fac, mass_2*time_fac
	# Replace mass by mass_1
	eta = (mass_1_1*mass_1_2)/(mass_1_1+mass_1_2)**2
	delta = (mass_1_1-mass_1_2)/(mass_1_1+mass_1_2)
	a_s = (a_1+a_2)/2.
	a_a = (a_1-a_2)/2.
	k1 = np.sqrt(1.-a_1**2)
	k2 = np.sqrt(1.-a_2**2)
	...
	B2_s = (B21+B22)/2.
	B2_a = (B21-B22)/2.
	v = (np.pi*(mass_1_1+mass_1_2)*f1)**(1./3.)
	con = 3./(128.*eta)/v**5
	    
	delta_psi_5 = -(10./9.)*((1.- 3.*eta)*a_s*(1 + 9.*a_a**2 + 3.*a_s**2) + delta*(1.-eta)*a_a*(1 + 3.*a_a**2 + 9.*a_s**2))
	
	delta_psi_5_l = 3.*delta_psi_5
	
	delta_psi_7 = (5./168.)*(delta*a_a*(-1667. - 4371.*a_a**2 - 13113.*a_s**2 + 616.*eta**2*(1 + 3.*a_a**2 + 9.*a_s**2) + 5.*eta*(311. + 807.*a_a**2 +2421.*a_s**2)) + a_s*(840.*eta**2*(9.*a_a**2 + 3.*a_s**2 + 1.) + eta*(38331.*a_a**2 + 12777.*a_s**2 + 4889.) - 13113.*a_a**2 - 4371.*a_s**2 - 1667.))
	
	delta_psi_8_a = -(5./27.)*(144.*np.pi*delta*(eta-1.)*a_a**3 + 48.*np.pi*delta*(eta-1)*a_a + 3.*(278.*eta**2 - 370.*eta + 75.)*a_a**4 + (-36.*eta**2 + 213.*eta - 67.)*a_a**2 + a_s*(-12.*delta*(eta**2 + 190.*eta - 75.)*a_a**3 + 2.*delta*(10.*eta**2 + 124.*eta - 67.)*a_a + 432.*np.pi*(3.*eta -1)*a_a**2 + 48.*np.pi*(3.*eta - 1)) + a_s**2*(432.*np.pi*delta*(eta -1)*a_a + 90.*(36.*eta**2 - 62.*eta + 15.)*a_a**2 - 172.*eta**2 + 303.*eta - 67.) + a_s**3*(12.*delta*(21.*eta**2 - 130.*eta + 75.)*a_a + 144.*np.pi*(3.*eta - 1)) + 3.*(82.*eta**2 -250.*eta + 75.)*a_s**4 - 12.*(2.*eta**2 - 4.*eta + 1.))
	
	delta_psi_8_b = -(20./9.) * ((delta*(2.*eta - 1.)*k_a + (-1. - 2.*eta**2 + 4.*eta)*k_s)*(1. + 6.*a_a**4 + 13.*a_s**2 + 6.*a_s**4 + a_a**2*(13. + 36.*a_s**2)) - 2.*(k_a*(1. - 4.*eta + 2.*eta**2) + delta*(1. - 2.*eta)*k_s)*a_a*a_s*(13. + 12.*(a_a**2 + a_s**2)))
	
	delta_psi_8_c = (80./9.)*(B2_s*((2.*eta**2 - 4.*eta + 1.)*a_s*(9.*a_a**2 + 3.*a_s**2 +1.) - delta*(2.*eta - 1.)*a_a*(3.*a_a**2 + 9.*a_s**2 + 1.)) + B2_a*(3.*(2.*eta**2 - 4.*eta + 1.)*a_a**3 + 9.*delta*(1. - 2.*eta)*a_a**2*a_s + (2.*eta**2 - 4.*eta + 1.)*a_a*(9.*a_s**2 + 1.) - delta*(2.*eta - 1.)*a_s*(3.*a_s**2 + 1.)))
	
	delta_psi_8 = delta_psi_8_a + delta_psi_8_b + delta_psi_8_c
	
	delta_psi_8_l = -3.*delta_psi_8
	
	delta_psi_addition_val = con*(delta_psi_5*v**5 + delta_psi_5_l*v**5*np.log(v) + delta_psi_7*v**7 + delta_psi_8*v**8 + delta_psi_8_l*v**8*np.log(v))
	
	return delta_psi_addition_val
	
	
def TH_phase_corr_wo_psi5_psi_8(f1, mass_1, mass_2, a_1, a_2):
	mass_1_1, mass_1_2 = mass_1*time_fac, mass_2*time_fac
	# Replace mass by mass_1
	eta = (mass_1_1*mass_1_2)/(mass_1_1+mass_1_2)**2
	delta = (mass_1_1-mass_1_2)/(mass_1_1+mass_1_2)
...
# 	delta_psi_addition_val = con*(delta_psi_5*v**5 + delta_psi_5_l*v**5*np.log(v) + delta_psi_7*v**7 + delta_psi_8*v**8 + delta_psi_8_l*v**8*np.log(v))
	...
	delta_psi_addition_val_wo_psi5_psi_8 = con*(delta_psi_5_l*v**5*np.log(v) + delta_psi_7*v**7 + delta_psi_8_l*v**8*np.log(v))
	
	return delta_psi_addition_val_wo_psi5_psi_8


# def lal_binary_black_hole_tidal_heating(
#         frequency_array, mass_1, mass_2, luminosity_distance, a_1, tilt_1,
#         phi_12, a_2, tilt_2, phi_jl, theta_jn, phase, H_eff5, H_eff8, **kwargs):
def CBC_TH_IMRPhenomD_Horizon(
        frequency_array, mass_1, mass_2, luminosity_distance, a_1, tilt_1,
    ...
    """
#     waveform_kwargs = dict(
#         waveform_approximant='TaylorF2_TidalHeated', reference_frequency=20.0, sampling_frequency=4096.0,
#         minimum_frequency=20.0, maximum_frequency=get_f_lso(mass_1, mass_2, a_1 * np.cos(tilt_1), a_2 * np.cos(tilt_2)),
#         catch_waveform_errors=False, pn_spin_order=-1, pn_tidal_order=-1,
#         pn_phase_order=-1, pn_amplitude_order=0)
#     waveform_kwargs.update(kwargs)
    
    waveform_kwargs = dict(
        waveform_approximant='IMRPhenomD_Horizon_TidalHeated', reference_frequency=50.0,
        minimum_frequency=20.0, maximum_frequency=0.018/((mass_1+mass_2)*time_fac),
        catch_waveform_errors=False, pn_spin_order=-1, pn_tidal_order=-1,
        pn_phase_order=-1, pn_amplitude_order=0)
    waveform_kwargs.update(kwargs)
    
#     return _base_lal_cbc_fd_waveform_TH(
#         frequency_array=frequency_array, mass_1=mass_1, mass_2=mass_2,
#         luminosity_distance=luminosity_distance, theta_jn=theta_jn, phase=phase,
#         H_eff5=H_eff5, H_eff8=H_eff8,
#         a_1=a_1, a_2=a_2, tilt_1=tilt_1, tilt_2=tilt_2, phi_12=phi_12,
#         phi_jl=phi_jl, **waveform_kwargs)

    return _base_lal_CBC_TH_IMRPhenomD_Horizon(
        frequency_array=frequency_array, mass_1=mass_1, mass_2=mass_2,
        luminosity_distance=luminosity_distance, a_1=a_1, tilt_1=tilt_1, phi_12=phi_12, a_2=a_2, tilt_2=tilt_2, phi_jl=phi_jl,theta_jn=theta_jn, phase=phase,
        dH=dH, **waveform_kwargs)


# def _base_lal_cbc_fd_waveform_TH(
#         frequency_array, mass_1, mass_2, luminosity_distance, theta_jn, phase, H_eff5, H_eff8,
#         a_1=0.0, a_2=0.0, tilt_1=0.0, tilt_2=0.0, phi_12=0.0, phi_jl=0.0,
#         lambda_1=0.0, lambda_2=0.0, eccentricity=0.0, **waveform_kwargs):
def _base_lal_CBC_TH_IMRPhenomD_Horizon(
        frequency_array, mass_1, mass_2, luminosity_distance, a_1, tilt_1,
        phi_12, a_2, tilt_2, phi_jl, theta_jn, phase, dH, **waveform_kwargs):
    """ Generate a cbc waveform model using lalsimulation
    ...
    phi_jl: float
        Azimuthal angle between the total and orbital angular momenta
    H_eff5: float
        2.5 PN effective tidal heating parameter
    H_eff8: float
        4 PN effective tidal heating parameter
    theta_jn: float
        Orbital inclination
    phase: float
        The phase at reference frequency or peak amplitude (depends on waveform)
    ...
    if waveform_approximant == 'IMRPhenomD_Horizon_TidalHeated':
        waveform_approximant = 'IMRPhenomD_Horizon'
    else:
        raise ValueError('Pass IMRPhenomD_Horizon_TidalHeated as approximant in waveform_kwargs dict')
    ...
    maxIndx = int(maximum_frequency / delta_frequency)+1
    frequency_array_1 = frequency_array[minIndx:maxIndx]
    delta_psi_addition = TH_phase_corr(frequency_array_1, mass_1, mass_2, a_1, a_2)
    delta_psi_addition_wo_psi5_psi_8 = TH_phase_corr_wo_psi5_psi_8(frequency_array_1, mass_1, mass_2, a_1, a_2)
...
#         waveform_dictionary, float(lambda_2))
    ...
    try:
        hplus, hcross = wf_func(
            ...
            start_frequency, maximum_frequency, reference_frequency,
            waveform_dictionary, approximant)
#         heated_phase = (dH)*delta_psi_addition
        heated_phase = - delta_psi_addition + (1+dH)*delta_psi_addition_wo_psi5_psi_8
#         heated_phase = - delta_psi_addition + (1+dH)*delta_psi_addition
        expo_heated_phase = np.cos(heated_phase) - 1j * np.sin(heated_phase)
        hplus.data.data[minIndx:maxIndx] = hplus.data.data[minIndx:maxIndx] * expo_heated_phase
        hcross.data.data[minIndx:maxIndx] = hcross.data.data[minIndx:maxIndx] * expo_heated_phase
    except Exception as e:
        if not catch_waveform_errors:
            raise
        else:
...
PARAMETER_SETS = dict(
    ...
    precession_only=precession_only,
    sky=sky, distance_inclination=distance_inclination,
    measured_spin=measured_spin, tidal_heating=tidal_heating
)


# Taylor_F2_Tidal_Heated
...
#     waveform_kwargs.update(kwargs)
    ...
    waveform_kwargs = dict(
        waveform_approximant='TaylorF2_TidalHeated', reference_frequency=50.0,
        minimum_frequency=20.0, maximum_frequency=0.018/((mass_1+mass_2)*time_fac),
        catch_waveform_errors=False, pn_spin_order=-1, pn_tidal_order=-1,
        pn_phase_order=-1, pn_amplitude_order=0)
    waveform_kwargs.update(kwargs)
...
#         phi_jl=phi_jl, **waveform_kwargs)
    ...
    return _base_lal_CBC_TH_TaylorF2(
        frequency_array=frequency_array, mass_1=mass_1, mass_2=mass_2,
        luminosity_distance=luminosity_distance, a_1=a_1, tilt_1=tilt_1, phi_12=phi_12, a_2=a_2, tilt_2=tilt_2, phi_jl=phi_jl,theta_jn=theta_jn, phase=phase,
        dH=dH, **waveform_kwargs)
...
def _base_lal_CBC_TH_TaylorF2(
    ...
    if waveform_approximant == 'TaylorF2_TidalHeated':
        waveform_approximant = 'TaylorF2'
    else:
        raise ValueError('Pass TaylorF2_TidalHeated as approximant in waveform_kwargs dict')
    ...
    delta_frequency = frequency_array[1] - frequency_array[0]
            #############################
#     maximum_frequency=get_f_lso(mass_1, mass_2, a_1 * np.cos(tilt_1), a_2 * np.cos(tilt_2))
#     start_frequency_1 = 0
#     minIndx = int(start_frequency_1 / delta_frequency)
    minIndx = int(start_frequency / delta_frequency)
    maxIndx = int(maximum_frequency / delta_frequency)+1
#     maxVal=len(frequency_array)-maxIndx
    frequency_array_1 = frequency_array[minIndx:maxIndx]
#     delta_psi_addition = TH_phase_corr(frequency_array_1, mass_1, mass_2, a_1, a_2)
    delta_psi_addition_wo_psi5_psi_8 = TH_phase_corr_wo_psi5_psi_8(frequency_array_1, mass_1, mass_2, a_1, a_2)
...
#         waveform_dictionary, float(lambda_2))
    ...
    try:
        hplus, hcross = wf_func(
            ...
            start_frequency, maximum_frequency, reference_frequency,
            waveform_dictionary, approximant)
#         heated_phase = (1+dH)*delta_psi_addition
        heated_phase = (1+dH)*delta_psi_addition_wo_psi5_psi_8
        expo_heated_phase = np.cos(heated_phase) - 1j * np.sin(heated_phase)
        # hpa=hplus.data.data*frequency_bounds
#         hca=hcross.data.data*frequency_bounds
#         hplus.data.data[:] = hpa * expo_heated_phase
#         hcross.data.data[:] = hca * expo_heated_phase
        hplus.data.data[minIndx:maxIndx] = hplus.data.data[minIndx:maxIndx] * expo_heated_phase
        hcross.data.data[minIndx:maxIndx] = hcross.data.data[minIndx:maxIndx] * expo_heated_phase
    except Exception as e:
        if not catch_waveform_errors:
